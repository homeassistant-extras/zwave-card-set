...
const splitNodes = (
  ...
  const sleepingNodes = hasFeature(config, 'hide-sleeping')
    ? []
    : nodes
 ...
 * const { deadNodes, liveNodes, asleepNodes } = getZWaveNodes(hass, config);
 *
...
export const getZWaveNodes = (
  ...
  return splitNodes(zWaveNodes, config);
};
